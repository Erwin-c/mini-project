...
static int readresp(char c, netbuf *nControl) {
  ...
  if (readline(nControl->response, RESPONSE_BUFSIZ, nControl) == -1) {
    if (ftplib_debug) perror("Control socket read failed");
    return 0;
  }
...
GLOBALDEF int FtpConnect(const char *host, netbuf **nControl) {
  ...
  if (isdigit(*pnum))
    ...
    int i;
    if ((i = getservbyname_r(pnum, "tcp", &se, tmpbuf, TMP_BUFSIZ, &pse)) !=
        0) {
      errno = i;
      if (ftplib_debug) perror("getservbyname_r");
      free(lhost);
      return 0;
    }
#else
    if ((pse = getservbyname(pnum, "tcp")) == NULL) {
      if (ftplib_debug) perror("getservbyname");
      free(lhost);
      return 0;
    }
#endif
  ...
  if ((sin.sin_addr.s_addr = inet_addr(lhost)) == INADDR_NONE) {
    ...
    char tmpbuf[TMP_BUFSIZ];
    int i, herr;
    if (((i = gethostbyname_r(lhost, &he, tmpbuf, TMP_BUFSIZ, &phe, &herr)) !=
         0) ||
        (phe == NULL)) {
      if (ftplib_debug) fprintf(stderr, "gethostbyname: %s\n", hstrerror(herr));
      free(lhost);
      return 0;
    }
#else
  ...
  if (sControl == -1) {
    if (ftplib_debug) perror("socket");
    return 0;
  }
  if (setsockopt(sControl, SOL_SOCKET, SO_REUSEADDR,
                 SETSOCKOPT_OPTVAL_TYPE & on, sizeof(on)) == -1) {
    if (ftplib_debug) perror("setsockopt");
    net_close(sControl);
    return 0;
  }
  if (connect(sControl, (struct sockaddr *)&sin, sizeof(sin)) == -1) {
    if (ftplib_debug) perror("connect");
    net_close(sControl);
    return 0;
  }
  ctrl = calloc(1, sizeof(netbuf));
  if (ctrl == NULL) {
    if (ftplib_debug) perror("calloc");
    net_close(sControl);
    return 0;
  }
...
static int FtpOpenPort(netbuf *nControl, netbuf **nData, int mode, int dir) {
  ...
  if (nControl->cmode == FTPLIB_PASSIVE) {
    ...
    if (getsockname(nControl->handle, &sin.sa, &l) < 0) {
      if (ftplib_debug) perror("getsockname");
      return -1;
    }
  }
  ...
  if (nControl->cmode == FTPLIB_PASSIVE) {
    ...
    if (bind(sData, &sin.sa, sizeof(sin)) == -1) {
      if (ftplib_debug) perror("bind");
      net_close(sData);
      return -1;
    }
    if (listen(sData, 1) < 0) {
      if (ftplib_debug) perror("listen");
      net_close(sData);
      return -1;
    }
...
static int FtpXfer(const char *localfile, const char *path, netbuf *nControl,
  ...
  if (!FtpAccess(path, typ, mode, nControl, &nData)) {
    if (localfile) {
      fclose(local);
      if (typ == FTPLIB_FILE_READ) unlink(localfile);
    }
    return 0;
  }
  ...
  if (typ == FTPLIB_FILE_WRITE) {
    ...
    while ((l = FtpRead(dbuf, FTPLIB_BUFSIZ, nData)) > 0) {
      if (fwrite(dbuf, 1, l, local) == 0) {
        if (ftplib_debug) perror("localfile write");
        rv = 0;
        break;
      }
    }
 ...
 * return 1 if successful, 0 otherwise
 */
GLOBALDEF int FtpSize(const char *path, unsigned int *size, char mode,
                      netbuf *nControl) {
  char cmd[TMP_BUFSIZ];
  int resp, rv = 1;
  unsigned int sz;
 ...
 * return 1 if successful, 0 otherwise
 */
GLOBALDEF int FtpSizeLong(const char *path, fsz_t *size, char mode,
                          netbuf *nControl) {
  char cmd[TMP_BUFSIZ];
  int resp, rv = 1;
  fsz_t sz;
