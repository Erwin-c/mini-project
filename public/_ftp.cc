...
bool Cftp::login(const char *host, const char *username, const char *password,
                 const int imode) {
  ...
  if (!FtpConnect(host, &m_ftpconn)) {
    m_connectfailed = true;
    return false;
  }

  if (!FtpLogin(username, password, m_ftpconn)) {
    m_loginfailed = true;
    return false;
  }

  if (!FtpOptions(FTPLIB_CONNMODE, (long)imode, m_ftpconn)) {
    m_optionfailed = true;
    return false;
  }
...
bool Cftp::get(const char *remotefilename, const char *localfilename,
  ...
  // 取文件.
  if (!FtpGet(strlocalfilenametmp, remotefilename, FTPLIB_IMAGE, m_ftpconn)) {
    return false;
  }

  // 判断文件获取前和获取后的时间, 如果时间不同,
  // 表示在文件传输的过程中已发生了变化, 返回失败.
  if (bCheckMTime) {
    char strmtime[21];
...
bool Cftp::put(const char *localfilename, const char *remotefilename,
               const bool bCheckSize) {
  if (m_ftpconn == nullptr) {
    return false;
  }
  ...
  // 发送文件.
  if (!FtpPut(localfilename, strremotefilenametmp, FTPLIB_IMAGE, m_ftpconn)) {
    return false;
  }

  // 重命名文件.
  if (!FtpRename(strremotefilenametmp, remotefilename, m_ftpconn)) {
    return false;
  }
  ...
  if (bCheckSize) {
    ...
    if (m_size != (unsigned int)FileSize(localfilename)) {
      return false;
    }
  }
...
bool Cftp::ftpdelete(const char *remotefilename) {
  ...
  if (!FtpDelete(remotefilename, m_ftpconn)) {
    return false;
  }
...
bool Cftp::ftprename(const char *srcremotefilename,
                     const char *dstremotefilename) {
  if (m_ftpconn == nullptr) {
    return false;
  }
