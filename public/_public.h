...
class CTcpClient {
 public:
  int m_connfd;     // 客户端的 Socket.
  char m_ip[21];    // 服务端的 IP 地址.
  int m_port;       // 与服务端通讯的端口.
  bool m_btimeout;  // 调用 Read() 方法时, 失败的原因是否是超时:
                    // true-超时; false-未超时.
  int m_buflen;  // 调用 Read() 方法后, 接收到的报文的大小, 单位: 字节.

  CTcpClient();   // 构造函数.
  ~CTcpClient();  // 析构函数自动关闭 Socket, 释放资源.
  ...
  // 接收服务端发送过来的数据.
  // buffer: 接收数据缓冲区的地址, 数据的长度存放在 m_buflen 成员变量中.
  // itimeout: 等待数据的超时时间, 单位: 秒, 缺省值是 0-无限等待.
  // 返回值: true-成功; false-失败, 失败有两种情况:
  //        1) 等待超时, 成员变量 m_btimeout 的值被设置为 true;
  //        2) Socket 连接已不可用.
...
class CTcpServer {
 public:
  ...
  CTcpServer();   // 构造函数.
  ~CTcpServer();  // 析构函数自动关闭 Socket, 释放资源.

  // 服务端初始化.
  // port: 指定服务端用于监听的端口.
  // 返回值: true-成功; false-失败, 一般情况下, 只要 port 设置正确,
  //                              没有被占用, 初始化都会成功.
  ...
  // 获取客户端的 IP 地址.
  // 返回值: 客户端的 IP 地址, 如 "192.168.1.100".
  char *GetIP();
  ...
  // 关闭监听的 Socket, 即 m_listenfd, 常用于多进程服务程序的子进程代码中.
  void CloseListen();

  // 关闭客户端的 Socket, 即 m_connfd, 常用于多进程服务程序的父进程代码中.
  void CloseClient();

 private:
  int m_socklen;             // 结构体 sockaddr_in 的大小.
  sockaddr_in m_clientaddr;  // 客户端的地址信息.
  sockaddr_in m_servaddr;    // 服务端的地址信息.
};
