/*
 * _mysql.cc, 此程序是开发框架的 C/C++ 操作 MySQL 数据库的定义文件.
 *
 *  Author: Erwin
 */
...
int sqlstatement::bindin(unsigned int position, int *value) {
  ...
  if ((position < 1) || (position >= MAXPARAMS) ||
      (position > m_handle->param_count)) {
    m_cda.rc = -1;
...
int sqlstatement::bindinlob(unsigned int position, void *buffer,
                            unsigned long *size) {
  if (m_state == 0) {
    m_cda.rc = -1;
    strncpy(m_cda.message, "cursor not open.\n", 128);
...
int sqlstatement::bindout(unsigned int position, char *value,
                          unsigned int len) {
  if (m_state == 0) {
    m_cda.rc = -1;
    strncpy(m_cda.message, "cursor not open.\n", 128);
...
int sqlstatement::bindoutlob(unsigned int position, void *buffer,
                             unsigned long buffersize, unsigned long *size) {
  if (m_state == 0) {
    m_cda.rc = -1;
    strncpy(m_cda.message, "cursor not open.\n", 128);
    return -1;
