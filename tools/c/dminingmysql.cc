/*
 * dminingmysql.cc, 本程序是数据中心的公共功能模块,
 *                  用于从 MySQL 数据库源表抽取数据, 生成 XML 文件.
 *
 * Author: Erwin
 */
...
struct st_arg {
  ...
  char selectsql[1024];  // 从数据源数据库抽取数据的 SQL 语句.
  char fieldstr[501];    // 抽取数据的 SQL 语句输出结果集字段名,
                         // 字段名之间用逗号分隔.
...
// 结果集字段值的最大长度, 存放 fieldlen 数组中元素的最大值.
int MAXFIELDLEN = -1;

// 结果集字段名数组, 从 starg.fieldstr 解析得到.
char strfieldname[MAXFIELDCOUNT][31];
// 结果集字段的长度数组, 从 starg.fieldlen 解析得到.
int ifieldlen[MAXFIELDCOUNT];
// strfieldname 和 ifieldlen 数组中有效字段的个数.
int ifieldcount;
// 递增字段在结果集数组中的位置.
int incfieldpos = -1;
...
bool _xmltoarg(char *strxmlbuffer) {
  ...
  // 判断字段数是否超出 MAXFIELDCOUNT 的限制.
  if (CmdStr.CmdCount() > MAXFIELDCOUNT) {
    logfile.Write("fieldstr 的字段数太多, 超出了最大限制 %d.\n", MAXFIELDCOUNT);
    return false;
  }
...
void _help() {
  ...
  printf(
      ...
      "keyid>:1 and ddatetime>timestampadd(minute,-120,now())</selectsql>"
  ...
  printf(
      "本程序是数据中心的公共功能模块, 用于从 MySQL 数据库源表抽取数据, 生成 "
      "XML 文件.\n");
  ...
  printf(
      "charset     数据库的字符集, "
      "这个参数要与数据源数据库保持一致，否则会出现中文乱码的情况.\n");
  // printf(
  //     "selectsql   "
  //     "从数据源数据库抽取数据的 SQL 语句, 注意: 时间函数的百分号%"
  //     "需要四个, 显示出来才有两个, 被 prepare() 之后将剩一个.\n");
  printf(
      "fieldstr    抽取数据的 SQL 语句输出结果集字段名, 中间用逗号分隔, 将作为 "
      "XML 文件的字段名.\n");
  printf(
      "fieldlen    "
      "抽取数据的 SQL 语句输出结果集字段的长度, 中间用逗号分隔. fieldstr 与 "
      "fieldlen 的字段必须一一对应.\n");
  printf("bfilename   输出 XML 文件的前缀.\n");
  printf("efilename   输出 XML 文件的后缀.\n");
  printf("outpath     输出 XML 文件存放的目录.\n");
  printf(
      "starttime   程序运行的时间区间, 例如 02,13表示: 如果程序启动时, 踏中 "
      "02时 和 13时 则运行, 其它时间不运行. 如果 starttime 为空, 那么 "
      "starttime 参数将失效, 只要本程序启动就会执行数据抽取, "
      "为了减少数据源的压力, 从数据库抽取数据的时候, "
      "一般在对方数据库最闲的时候时进行.\n");
  printf(
      "incfield    递增字段名, 它必须是 fieldstr 中的字段名, 并且只能是整型, "
      "一般为自增字段. 如果 incfield 为空, 表示不采用增量抽取方案.");
  printf(
      "incfilename 已抽取数据的递增字段最大值存放的文件, 如果该文件丢失, "
      "将重新抽取全部的数据.\n");
  printf("timeout     本程序的超时时间, 单位: 秒.\n");
  printf(
      "pname       进程名, 尽可能采用易懂的, 与其它进程不同的名称, "
      "方便故障排查.\n\n\n");
