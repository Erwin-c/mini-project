...
vector<struct st_fileinfo>
    vlistfile1;  // 已上传成功文件名的容器，从okfilename中加载。
vector<struct st_fileinfo>
    vlistfile2;  // 上传前列出客户端文件名的容器，从nlist文件中加载。
  ...
  for (size_t ii = 0; ii < vlistfile2.size(); ii++) {
    ...
    if (starg.ptype == 3) {
      ...
      SNPRINTF(strlocalfilenamebak, sizeof(strlocalfilenamebak), 300, "%s/%s",
               starg.localpathbak, vlistfile2[ii].filename);
...
void _help() {
  ...
  printf(
      "Sample:/project/tools1/bin/procctl 30 /project/tools1/bin/ftpputfiles "
      "/log/idc/ftpputfiles_surfdata.log "
      "\"<host>127.0.0.1:21</host><mode>1</mode><username>wucz</"
      "username><password>wuczpwd</password><localpath>/tmp/idc/surfdata</"
      "localpath><remotepath>/tmp/ftpputest</"
      "remotepath><matchname>SURF_ZH*.JSON</matchname><ptype>1</"
      "ptype><localpathbak>/tmp/idc/surfdatabak</localpathbak><okfilename>/"
      "idcdata/ftplist/ftpputfiles_surfdata.xml</okfilename><timeout>80</"
      "timeout><pname>ftpputfiles_surfdata</pname>\"\n\n\n");
  ...
  printf(
      "<remotepath>/tmp/ftpputest</remotepath> 远程服务端存放文件的目录。\n");
...
// 把 localpath 目录下的文件加载到 vlistfile2 容器中.
bool LoadLocalFile() {
  vlistfile2.clear();
  ...
  // 不包括子目录.
  // 注意, 如果本地目录下的总文件数超过 10000, 增量上传文件功能将有问题.
  // 建议用 deletefiles 程序及时清理本地目录中的历史文件.
  if (!Dir.OpenDir(starg.localpath, starg.matchname)) {
    logfile.Write("Dir.OpenDir(%s) falied.\n", starg.localpath);
    return false;
  }
