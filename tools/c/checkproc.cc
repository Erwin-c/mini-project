...
int main(int argc, char *argv[]) {
  ...
  if (argc != 2) {
    ...
    printf(
        ...
        "~/Coding/mini-project/tools/tmp/log/checkproc.log\n\n");
  ...
  for (int ii = 0; ii < MAXNUMP; ++ii) {
    ...
    if (iret == -1) {
      logfile.Write("Process pid=%d(%s) did not exist.\n", (shm + ii)->pid,
                    (shm + ii)->pname);
    ...
    // 如果已超时.
    logfile.Write("Process pid=%d(%s) timeout.\n", (shm + ii)->pid,
                  (shm + ii)->pname);

    // 发送信号15, 尝试正常终止进程.
    kill(shm[ii].pid, 15);

    // 每隔 1 秒判断一次进程是否存在, 累计5秒, 一般来说,
    // 5秒的时间足够让进程退出.
    ...
    if (iret == -1) {
      logfile.Write("Process pid=%d(%s) was terminated normally.\n",
                    (shm + ii)->pid, (shm + ii)->pname);
    } else {
      kill(shm[ii].pid, 9);  // 如果进程仍存在, 就发送信号9, 强制终止它.
      logfile.Write("Process pid=%d(%s) was terminated compulsorily\n",
                    (shm + ii)->pid, (shm + ii)->pname);
    }
